...
pub fn mul<T>(matrix: &Matrix<T>, other: &Matrix<T>) -> Matrix<T>
    ...
    strassen_mul(matrix, other, true)
}

fn strassen_mul<T>(matrix: &Matrix<T>, other: &Matrix<T>, multi_thread: bool) -> Matrix<T>
        where T: Default + Clone + ops::Add<Output = T> + ops::Sub<Output = T> + ops::Mul<Output = T> + marker::Send + 'static {
    ...
    let (p1, p2, p3, p4, p5, p6, p7) = if multi_thread {
        mul_multi_thread(&a11, &a12, &a21, &a22, &b11, &b12, &b21, &b22)
    } else {
        mul_single_thread(&a11, &a12, &a21, &a22, &b11, &b12, &b21, &b22)
    };
...
fn mul_single_thread<T>(a11: &Matrix<T>, a12: &Matrix<T>, a21: &Matrix<T>, a22: &Matrix<T>,
                     b11: &Matrix<T>, b12: &Matrix<T>, b21: &Matrix<T>, b22: &Matrix<T>) ->
                        (Matrix<T>, Matrix<T>, Matrix<T>, Matrix<T>, Matrix<T>, Matrix<T>, Matrix<T>)
        where T: Default + Clone + ops::Add<Output = T> + ops::Sub<Output = T> + ops::Mul<Output = T> + marker::Send + 'static {
    let p1 = strassen_mul(&(a11 + a22), &(b11 + b22), false);
    let p2 = strassen_mul(&(a21 + a22), b11, false);
    let p3 = strassen_mul(a11, &(b12 - b22), false);
    let p4 = strassen_mul(&a22, &(b21 - b11), false);
    let p5 = strassen_mul(&(a11 + a12), b22, false);
    let p6 = strassen_mul(&(a21 - a11), &(b11 + b12), false);
    let p7 = strassen_mul(&(a12 - a22), &(b21 + b22), false);
...
fn mul_multi_thread<T>(a11: &Matrix<T>, a12: &Matrix<T>, a21: &Matrix<T>, a22: &Matrix<T>,
    ...
    let child1 = thread::spawn(move || strassen_mul(&(&a11_c1 + &a22_c1), &(&b11_c1 + &b22_c1), false));

    let (a21_c2, a22_c2, b11_c2) = (a21.clone(), a22.clone(), b11.clone());
    let child2 = thread::spawn(move || strassen_mul(&(&a21_c2 + &a22_c2), &b11_c2, false));

    let (a11_c3, b12_c3, b22_c3) = (a11.clone(), b12.clone(), b22.clone());
    let child3 = thread::spawn(move || strassen_mul(&a11_c3, &(&b12_c3 - &b22_c3), false));

    let (a22_c4, b21_c4, b11_c4) = (a22.clone(), b21.clone(), b11.clone());
    let child4 = thread::spawn(move || strassen_mul(&a22_c4, &(&b21_c4 - &b11_c4), false));

    let (a11_c5, a12_c5, b22_c5) = (a11.clone(), a12.clone(), b22.clone());
    let child5 = thread::spawn(move || strassen_mul(&(&a11_c5 + &a12_c5), &b22_c5, false));

    let (a11_c6, a21_c6, b11_c6, b12_c6) = (a11.clone(), a21.clone(), b11.clone(), b12.clone());
    let child6 = thread::spawn(move || strassen_mul(&(&a21_c6 - &a11_c6), &(&b11_c6 + &b12_c6), false));

    let (a12_c7, a22_c7, b21_c7, b22_c7) = (a12.clone(), a22.clone(), b21.clone(), b22.clone());
    let child7 = thread::spawn(move || strassen_mul(&(&a12_c7 - &a22_c7), &(&b21_c7 + &b22_c7), false));
